...
function App() {
  ...
  return <main className={`app-shell ${exploded ? 'exploded' : ''}`}>
    ...
    {aboutOpen && <div className="about-backdrop" role="presentation" onClick={() => setAboutOpen(false)}><section className="about-card" role="dialog" aria-modal="true" aria-labelledby="about-title" onClick={(event) => event.stopPropagation()}><button className="round about-close" onClick={() => setAboutOpen(false)} aria-label="关闭档案"><X size={16} /></button><p className="eyebrow">THE MAKING OF A LEGEND</p><h2 id="about-title">传奇，逐层展开。</h2><p>以擎天柱的经典红蓝形象为起点，通过 Hyper3D Rodin 重建三维模型，并使用 8 个独立机械部件进行爆炸视图展示。</p><p>这是 AI 生成的角色视觉研究，不代表官方设定或真实变形机构。</p><div className="about-meta"><span>ASSET // OPT-01</span><span>PARTS // 08</span><span>ENGINE // RODIN</span></div></section></div>}
  </main>;
}
